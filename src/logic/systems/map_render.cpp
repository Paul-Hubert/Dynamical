...
void MapRenderSys::tick(float dt) {
    ...
    auto corner_pos = map.getChunkPos(camera.corner);
    auto end_pos = map.getChunkPos(camera.corner + camera.size);
        ...
        for(int y = corner_pos.y; y <= end_pos.y; y++) {
            ...
            for(int i = 0; i<stored_chunks.size(); i++) {
                if(stored_chunks[i].stored && stored_chunks[i].position == pos) {
                    stored = true;
                    header->chunk_indices[(pos.x - corner_pos.x) * header->chunk_length + (pos.y - corner_pos.y)] = i;
            ...
            if(!stored) {
                ...
                header->chunk_indices[(pos.x - corner_pos.x) * header->chunk_length + (pos.y - corner_pos.y)] = index;
