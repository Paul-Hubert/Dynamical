...
BuildingRenderSys::BuildingRenderSys(entt::registry& reg) : System(reg) {
...
void BuildingRenderSys::tick(double dt) {
    ...
    // Buildings
    ctx.classic_render.command.bindPipeline(vk::PipelineBindPoint::eGraphics, buildingPipeline);
    ...
    ctx.classic_render.command.bindVertexBuffers(0, {data.buildingBuffer}, {0});

    ctx.classic_render.command.draw(48, data.num_buildings, 0, 0);

}

void BuildingRenderSys::initPipeline() {
    ...
    auto vertShaderCode = dy::readFile("./resources/shaders/buildingrender.vert.glsl.spv");
    auto fragShaderCode = dy::readFile("./resources/shaders/buildingrender.frag.glsl.spv");
    ...
    auto vertexInputBindings = std::vector<vk::VertexInputBindingDescription> {
            vk::VertexInputBindingDescription(0, sizeof(RenderBuilding), vk::VertexInputRate::eInstance)
    };

    auto vertexInputAttributes = std::vector<vk::VertexInputAttributeDescription> {
            vk::VertexInputAttributeDescription(0, 0, vk::Format::eR32G32B32A32Sfloat, offsetof(RenderBuilding, position)),
            vk::VertexInputAttributeDescription(1, 0, vk::Format::eR32G32B32A32Sfloat, offsetof(RenderBuilding, dimensions)),
            vk::VertexInputAttributeDescription(2, 0, vk::Format::eR32G32B32A32Sfloat, offsetof(RenderBuilding, wall_color)),
            vk::VertexInputAttributeDescription(3, 0, vk::Format::eR32G32B32A32Sfloat, offsetof(RenderBuilding, roof_color))
    };

    auto vertexInputState = vk::PipelineVertexInputStateCreateInfo({}, (uint32_t) vertexInputBindings.size(), vertexInputBindings.data(), (uint32_t) vertexInputAttributes.size(), vertexInputAttributes.data());
    ...
    rasterizer.cullMode = VK_CULL_MODE_NONE;  // No back-face culling, same as objects
    ...
    buildingPipeline = ctx.device->createGraphicsPipeline(nullptr, {pipelineInfo}).value;
...
BuildingRenderSys::~BuildingRenderSys() {

    Context& ctx = *reg.ctx<Context*>();

    ctx.device->destroy(buildingPipeline);
